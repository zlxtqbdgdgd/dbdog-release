#!/usr/bin/env bash
# Apply or verify GaussDB's per-database DBM objects across the databases that
...
#
# 本仓是这个脚本的唯一 owning path：agent-install.sh 把它与 init-gaussdb-perdb.sql 一起装到
...
PERDB_SQL=${GAUSSDB_PERDB_SQL:-$SCRIPT_DIR/init-gaussdb-perdb.sql}
...
usage() {
  cat <<'EOF'
Usage:
  GAUSSDB_PORT=<port> [connection env...] /opt/dbdog-agent/scripts/init-dbdog-user-gaussdb-all-databases.sh --all
  GAUSSDB_PORT=<port> [connection env...] /opt/dbdog-agent/scripts/init-dbdog-user-gaussdb-all-databases.sh --check --all
  GAUSSDB_PORT=<port> [connection env...] /opt/dbdog-agent/scripts/init-dbdog-user-gaussdb-all-databases.sh [--check] DB [DB...]
...
Connection environment:
  ...
  GAUSSDB_ADMIN_DB    database used for --all enumeration (default: postgres)
  GAUSSDB_ADMIN_USER  administrative user passed to gsql with -U (optional)
  GAUSSDB_PERDB_SQL   per-database SQL file override (default: init-gaussdb-perdb.sql
                      next to this script)
...
echo "GaussDB per-database DBM setup $mode complete: ${#requested_databases[@]} database(s)"
