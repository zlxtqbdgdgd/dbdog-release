...
control_overlay_rel=control-overlays/4c39489b8c0b7fb7a46af88062fb9aadf2c08264-7a4247599b029f1aca10d2cb63491d535fbd502f-aarch64-kylin10-v7-omnibus-kylin-platform-v8
...
verify_patchelf_tool_authority() {
  ...
  if [[ ! -d $patchelf_tool_dir || -L $patchelf_tool_dir ]]; then
    echo "pinned v8 patchelf tool root must be a real directory: $patchelf_tool_dir" >&2
    exit 1
  fi
  actual_inventory=$(
    {
  ...
  if [[ $actual_inventory != "$expected_inventory" ]]; then
    echo "pinned v8 patchelf tool inventory does not match its five-node contract" >&2
    printf '%s\n' "$actual_inventory" >&2
    exit 1
  fi
  ...
  while IFS= read -r entry; do
    ...
    if [[ -L $entry_path ]]; then
      echo "pinned v8 patchelf tool entry must not be a symlink: $entry_path" >&2
      exit 1
    fi
    resolved_path=$(readlink -e -- "$entry_path" || :)
    if [[ $resolved_path != "$entry_path" ]]; then
      echo "pinned v8 patchelf tool entry is not canonical: $entry_path" >&2
      exit 1
    fi
    case $entry in
      . | bin)
        if [[ ! -d $entry_path ]]; then
          echo "pinned v8 patchelf tool entry must be a directory: $entry_path" >&2
          exit 1
        fi
      ...
      PATCHELF-INFO | SHA256SUMS)
        if [[ ! -f $entry_path ]]; then
          echo "pinned v8 patchelf metadata entry must be a regular file: $entry_path" >&2
          exit 1
        fi
      ...
      bin/patchelf)
        if [[ ! -f $entry_path ]]; then
          echo "pinned v8 patchelf binary must be a regular file: $entry_path" >&2
          exit 1
        fi
        expected_stat=root:root:555
        ;;
      *)
        echo "unexpected pinned v8 patchelf inventory entry: $entry" >&2
        exit 1
        ;;
    esac
    actual_stat=$(stat -c '%U:%G:%a' -- "$entry_path")
    if [[ $actual_stat != "$expected_stat" ]]; then
      echo "pinned v8 patchelf tool owner/mode mismatch: $entry_path ($actual_stat)" >&2
      exit 1
    fi
...
verify_patchelf_path_selection() {
  ...
  if [[ ${PATH%%:*} != "$patchelf_bin_dir" ]]; then
    echo "pinned v8 patchelf bin directory is not first in PATH" >&2
    exit 1
  fi
  selected_patchelf=$(command -v patchelf || :)
  if [[ $selected_patchelf != "$patchelf_binary" ]]; then
    echo "command -v patchelf did not select the pinned v8 binary: $selected_patchelf" >&2
    exit 1
  fi
}
...
verify_patchelf_runtime() (
  ...
  if [[ $version_output != "$patchelf_reported_version" ]]; then
    echo "pinned v8 patchelf reported an unexpected version: $version_output" >&2
    exit 1
  fi
  ...
  case $smoke_dir in
    ...
    *)
      echo "unsafe pinned v8 patchelf smoke directory: $smoke_dir" >&2
      exit 1
      ;;
  esac
  ...
  if [[ $actual_rpath != "$smoke_rpath" ]]; then
    echo "pinned v8 patchelf RPATH smoke test failed: $actual_rpath" >&2
    exit 1
  fi
...
verify_rpm_package_clean() {
  local package=$1 verify_output

  if ! verify_output=$(
    "$env_binary" -i PATH=/usr/bin:/bin LANG=C LC_ALL=C \
      "$rpm_binary" -V "$package" 2>&1
  ); then
    echo "RPM verification failed for pinned Kylin v8 package: $package" >&2
    printf '%s\n' "$verify_output" >&2
    exit 1
  fi
  if [[ -n $verify_output ]]; then
    echo "RPM verification reported changed files for pinned Kylin v8 package: $package" >&2
    printf '%s\n' "$verify_output" >&2
    exit 1
  fi
}
...
verify_pinned_selinux_system_file() {
  local label=$1 path=$2 expected_bytes=$3 expected_sha256=$4 expected_package=$5
  local resolved_path actual_stat expected_stat actual_package

  if [[ ! -f $path || -L $path ]]; then
    echo "pinned Kylin v8 $label must be a real regular file: $path" >&2
    exit 1
  fi
  resolved_path=$(readlink -e -- "$path" || :)
  if [[ $resolved_path != "$path" ]]; then
    echo "pinned Kylin v8 $label is not canonical: $path -> $resolved_path" >&2
    exit 1
  fi
  expected_stat="0:0:755:$expected_bytes"
  actual_stat=$(stat -c '%u:%g:%a:%s' -- "$path")
  if [[ $actual_stat != "$expected_stat" ]]; then
    echo "pinned Kylin v8 $label owner/mode/size mismatch: $actual_stat" >&2
    exit 1
  fi
  ...
  if [[ $actual_package != "$expected_package" ]]; then
    echo "pinned Kylin v8 $label RPM mismatch: $actual_package" >&2
    exit 1
  fi
}
...
verify_selinux_tool_identity() {
  ...
  if [[ ! -x $rpm_binary || -L $rpm_binary ]]; then
    echo "Kylin v8 requires the real RPM query tool: $rpm_binary" >&2
    exit 1
  fi
  if [[ ! -x $file_binary || -L $file_binary ]]; then
    echo "Kylin v8 requires the real file inspection tool: $file_binary" >&2
    exit 1
  fi
  ...
  verify_rpm_package_clean "$checkmodule_package"
  verify_rpm_package_clean "$semodule_package_package"
  verify_rpm_package_clean "$libsepol_package"

  if ! version_output=$(
    "$env_binary" -i PATH=/usr/bin:/bin LANG=C LC_ALL=C \
      "$checkmodule_binary" -V 2>&1
  ); then
    echo "pinned Kylin v8 checkmodule -V probe failed" >&2
    printf '%s\n' "$version_output" >&2
    exit 1
  fi
  if [[ $version_output != "$checkmodule_versions" ]]; then
    echo "pinned Kylin v8 checkmodule module-version range changed: $version_output" >&2
    exit 1
  fi
  ...
  case $help_status in
    0 | 1) ;;
    *)
      echo "pinned Kylin v8 checkmodule help probe failed with status $help_status" >&2
      printf '%s\n' "$help_output" >&2
      exit 1
      ;;
  esac
  for required_option in -h -V -b -C -U -m -M -o; do
    if ! printf '%s\n' "$help_output" | grep -F -- "$required_option" >/dev/null; then
      echo "pinned Kylin v8 checkmodule help omitted $required_option" >&2
      exit 1
    fi
  done
  if printf '%s\n' "$help_output" |
    grep -Eq -- '(^|[^[:alnum:]_])-c([^[:alnum:]_]|$)'; then
    echo "pinned Kylin v8 checkmodule unexpectedly advertises -c" >&2
    exit 1
  fi
}
...
verify_selinux_tool_path_selection() {
  ...
  if [[ $selected_checkmodule != "$checkmodule_binary" ]]; then
    echo "PATH did not select pinned Kylin v8 checkmodule: $selected_checkmodule" >&2
    exit 1
  fi
  selected_semodule_package=$(command -v semodule_package || :)
  if [[ $selected_semodule_package != "$semodule_package_binary" ]]; then
    echo "PATH did not select pinned Kylin v8 semodule_package: $selected_semodule_package" >&2
    exit 1
  fi
}
...
verify_selinux_policy_toolchain_smoke() (
  ...
  case $smoke_dir in
    ...
    *)
      echo "unsafe Kylin v8 SELinux smoke directory: $smoke_dir" >&2
      exit 1
      ;;
  esac
  ...
  if ! compile_output=$(
    ...
    echo "pinned Kylin v8 checkmodule policy smoke failed" >&2
    printf '%s\n' "$compile_output" >&2
    exit 1
  fi
  printf '%s\n' "$compile_output"
  if ! printf '%s\n' "$compile_output" |
    ...
    echo "pinned Kylin v8 checkmodule did not prove modular policy version 19" >&2
    exit 1
  fi
  if [[ ! -s $smoke_mod || -L $smoke_mod ]]; then
    echo "pinned Kylin v8 checkmodule produced no real nonempty module" >&2
    exit 1
  fi
  ...
  if ! package_output=$(
    ...
    echo "pinned Kylin v8 semodule_package smoke failed" >&2
    printf '%s\n' "$package_output" >&2
    exit 1
  fi
  ...
  if [[ ! -s $smoke_pp || -L $smoke_pp ]]; then
    echo "pinned Kylin v8 semodule_package produced no real nonempty package" >&2
    exit 1
  fi
  if ! file_output=$(
    "$env_binary" -i PATH=/usr/bin:/bin LANG=C LC_ALL=C \
    ...
    echo "pinned Kylin v8 file probe could not inspect the smoke policy package" >&2
    printf '%s\n' "$file_output" >&2
    exit 1
  fi
  if ! printf '%s\n' "$file_output" | grep -F -- 'mod version 19' >/dev/null; then
    echo "pinned Kylin v8 smoke package is not modular policy version 19: $file_output" >&2
    exit 1
  fi
  printf 'Kylin v8 SELinux smoke package: %s\n' "$file_output"
...
if [[ ${OMNIBUS_RUBY_VERSION:-$omnibus_ruby_sha} != "$omnibus_ruby_sha" ]]; then
  echo "OMNIBUS_RUBY_VERSION does not match the v8 build input" >&2
  exit 1
fi
...
if [[ $host_glibc != 'glibc 2.28' ]]; then
  echo "Kylin v8 requires native glibc 2.28, found: $host_glibc" >&2
  exit 1
fi
...
if [[ "$actual_overlay_inventory" != "$expected_overlay_inventory" ]]; then
  echo "Omnibus control overlay inventory does not match the pinned v8 inventory" >&2
...
if [[ $platform_patch_sha256 != "$platform_patch_expected_sha256" ]]; then
  echo "Kylin v8 combined patch does not match its pinned SHA-256" >&2
  exit 1
fi
verify_patchelf_tool_authority
base_patch_names=(
...
ensure_platform_patch_applied() {
  if git apply --check "$platform_patch" >/dev/null 2>&1; then
    ...
    echo "Kylin v8 platform/toolchain patch is neither cleanly applicable nor already applied" >&2
    exit 1
  fi
}
