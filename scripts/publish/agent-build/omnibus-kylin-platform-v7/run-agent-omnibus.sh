...
control_overlay_rel=control-overlays/4c39489b8c0b7fb7a46af88062fb9aadf2c08264-7a4247599b029f1aca10d2cb63491d535fbd502f-aarch64-kylin10-v7-omnibus-kylin-platform-v7
...
platform_patch_expected_sha256=2fd56423b5764772436f887e75f6751af535f71d3d9985bac3b8f5a01ad3ee0d
...
verify_patchelf_tool_authority() {
  ...
  if [[ ! -d $patchelf_tool_dir || -L $patchelf_tool_dir ]]; then
    echo "pinned v7 patchelf tool root must be a real directory: $patchelf_tool_dir" >&2
    exit 1
  fi
  actual_inventory=$(
    {
  ...
  if [[ $actual_inventory != "$expected_inventory" ]]; then
    echo "pinned v7 patchelf tool inventory does not match its five-node contract" >&2
    printf '%s\n' "$actual_inventory" >&2
    exit 1
  fi
  ...
  while IFS= read -r entry; do
    ...
    if [[ -L $entry_path ]]; then
      echo "pinned v7 patchelf tool entry must not be a symlink: $entry_path" >&2
      exit 1
    fi
    resolved_path=$(readlink -e -- "$entry_path" || :)
    if [[ $resolved_path != "$entry_path" ]]; then
      echo "pinned v7 patchelf tool entry is not canonical: $entry_path" >&2
      exit 1
    fi
    case $entry in
      . | bin)
        if [[ ! -d $entry_path ]]; then
          echo "pinned v7 patchelf tool entry must be a directory: $entry_path" >&2
          exit 1
        fi
      ...
      PATCHELF-INFO | SHA256SUMS)
        if [[ ! -f $entry_path ]]; then
          echo "pinned v7 patchelf metadata entry must be a regular file: $entry_path" >&2
          exit 1
        fi
      ...
      bin/patchelf)
        if [[ ! -f $entry_path ]]; then
          echo "pinned v7 patchelf binary must be a regular file: $entry_path" >&2
          exit 1
        fi
        expected_stat=root:root:555
        ;;
      *)
        echo "unexpected pinned v7 patchelf inventory entry: $entry" >&2
        exit 1
        ;;
    esac
    actual_stat=$(stat -c '%U:%G:%a' -- "$entry_path")
    if [[ $actual_stat != "$expected_stat" ]]; then
      echo "pinned v7 patchelf tool owner/mode mismatch: $entry_path ($actual_stat)" >&2
      exit 1
    fi
...
verify_patchelf_path_selection() {
  ...
  if [[ ${PATH%%:*} != "$patchelf_bin_dir" ]]; then
    echo "pinned v7 patchelf bin directory is not first in PATH" >&2
    exit 1
  fi
  selected_patchelf=$(command -v patchelf || :)
  if [[ $selected_patchelf != "$patchelf_binary" ]]; then
    echo "command -v patchelf did not select the pinned v7 binary: $selected_patchelf" >&2
    exit 1
  fi
}
...
verify_patchelf_runtime() (
  ...
  if [[ $version_output != "$patchelf_reported_version" ]]; then
    echo "pinned v7 patchelf reported an unexpected version: $version_output" >&2
    exit 1
  fi
  ...
  case $smoke_dir in
    ...
    *)
      echo "unsafe pinned v7 patchelf smoke directory: $smoke_dir" >&2
      exit 1
      ;;
  esac
  ...
  if [[ $actual_rpath != "$smoke_rpath" ]]; then
    echo "pinned v7 patchelf RPATH smoke test failed: $actual_rpath" >&2
    exit 1
  fi
...
if [[ $host_glibc != 'glibc 2.28' ]]; then
  echo "Kylin v7 requires native glibc 2.28, found: $host_glibc" >&2
  exit 1
fi
...
if [[ "$actual_overlay_inventory" != "$expected_overlay_inventory" ]]; then
  echo "Omnibus control overlay inventory does not match the pinned v7 inventory" >&2
...
if [[ $platform_patch_sha256 != "$platform_patch_expected_sha256" ]]; then
  echo "Kylin v7 combined patch does not match its pinned SHA-256" >&2
  exit 1
fi
verify_patchelf_tool_authority
base_patch_names=(
...
ensure_platform_patch_applied() {
  if git apply --check "$platform_patch" >/dev/null 2>&1; then
    ...
    echo "Kylin v7 platform/toolchain patch is neither cleanly applicable nor already applied" >&2
    exit 1
  fi
}
