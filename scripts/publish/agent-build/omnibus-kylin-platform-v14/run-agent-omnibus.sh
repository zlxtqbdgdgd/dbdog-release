...
case ${1:-} in
  --resume-v9-retry6-post-health)
    echo "v14 is a fresh-build-only control and cannot resume historical v9 work" >&2
    exit 1
    ;;
  --adopt-post-health-v2)
    echo "v14 is a fresh-build-only control and cannot adopt historical v12 work" >&2
    exit 1
    ;;
esac
if (($# > 1)); then
  echo "usage: run-agent-omnibus.sh [build-dir]" >&2
  exit 1
fi
build_dir=${BUILD_DIR:-${1:-/home/dbdog/work/dbdog-agent-62ad2979-build2}}
build_dir=$(readlink -f -- "$build_dir")
case "$build_dir" in
  /home/dbdog/work/dbdog-agent-62ad2979-build*) ;;
  *)
...
control_overlay_rel=control-overlays/62ad29793b02139448b76bc85fc406491a08bf58-7a4247599b029f1aca10d2cb63491d535fbd502f-aarch64-kylin10-v7-omnibus-kylin-platform-v14
...
pipeline_lock="$pipeline_lock_dir/dbdog-agent-62ad2979-aarch64-kylin10.pipeline.lock"
agent_sha=62ad29793b02139448b76bc85fc406491a08bf58
generated_outputs_origin_agent_sha=4c39489b8c0b7fb7a46af88062fb9aadf2c08264
core_sha=7a4247599b029f1aca10d2cb63491d535fbd502f
release_version=${DBDOG_PACKAGE_VERSION:-}
if [[ ! $release_version =~ ^7[.]81[.]0-dbdog[.][1-9][0-9]*$ ]]; then
  echo "DBDOG_PACKAGE_VERSION must be an explicit 7.81.0-dbdog.N release version" >&2
  exit 1
fi
...
if [[ "$actual_overlay_inventory" != "$expected_overlay_inventory" ]]; then
    echo "Omnibus control overlay inventory does not match the pinned v14 inventory" >&2
...
prepare_release_version_cache() {
  ...
  printf '%s\n' \
    '{' \
    "    \"6\": [\"6.81.0\", \"dbdog.$release_revision\", 0, \"${agent_sha:0:7}\", null]," \
    "    \"7\": [\"7.81.0\", \"dbdog.$release_revision\", 0, \"${agent_sha:0:7}\", null]," \
    '    "nightly": false,' \
    '    "dev": false' \
    '}' \
    >"$version_cache"
