#!/usr/bin/bash
# Narrow root entry point for the pinned Kylin V13 AArch64 dbdog-agent
...
readonly EXPECTED_SELF=/home/dbdog/cache/dbdog-agent/controls/run-finalize-agent-runtime-v2.sh
readonly FINALIZER=/home/dbdog/cache/dbdog-agent/controls/finalize-agent-runtime-v2.sh
...
readonly CORE_SHA=d725d9847379ff919b60a2f35e1f41af001e6054
readonly MANIFEST_REL="manifests/$GENERATED_OUTPUTS_ORIGIN_AGENT_SHA-$OMNIBUS_CORE_SHA-aarch64-kylin10-v7"
readonly OVERLAY_REL="control-overlays/$AGENT_SHA-$OMNIBUS_CORE_SHA-aarch64-kylin10-v7-omnibus-kylin-platform-v13"
...
die() {
  printf '[run-finalize-agent-runtime-v2] ERROR: %s\n' "$*" >&2
  exit 1
}
...
(($# == 1)) || die 'usage: run-finalize-agent-runtime-v2.sh <7.81.0-dbdog.N>'
readonly VERSION=$1
...
[[ -d $OVERLAY_DIR && ! -L $OVERLAY_DIR ]] || die "missing v13 control overlay: $OVERLAY_DIR"
[[ $(/usr/bin/readlink -e -- "$OVERLAY_DIR") == "$OVERLAY_DIR" ]] || \
  die 'v13 control overlay resolves through an unexpected path'
[[ $(/usr/bin/stat -c '%u:%g:%a' -- "$OVERLAY_DIR") == 0:0:555 ]] || \
  die 'v13 control overlay must be root:root mode 0555'
...
[[ $overlay_inventory == "$expected_overlay_inventory" ]] || \
  die 'v13 control-overlay inventory differs from the pinned four-file set'
for overlay_data in CONTROL-INFO CONTROL.sha256 agent-build-kylin-platform.patch; do
  [[ -f $OVERLAY_DIR/$overlay_data && ! -L $OVERLAY_DIR/$overlay_data ]] || \
    die "v13 control is not a regular file: $overlay_data"
  [[ $(/usr/bin/stat -c '%u:%g:%a' -- "$OVERLAY_DIR/$overlay_data") == 0:0:444 ]] || \
    die "v13 control must be root:root mode 0444: $overlay_data"
done
[[ -f $OVERLAY_DIR/run-agent-omnibus.sh && ! -L $OVERLAY_DIR/run-agent-omnibus.sh ]] || \
  die 'v13 runner is not a regular file'
[[ $(/usr/bin/stat -c '%u:%g:%a' -- "$OVERLAY_DIR/run-agent-omnibus.sh") == 0:0:555 ]] || \
  die 'v13 runner must be root:root mode 0555'
if ! /usr/bin/cmp -s -- "$OVERLAY_DIR/CONTROL.sha256" <(
  ...
  die 'v13 CONTROL.sha256 differs from the pinned four-entry manifest'
fi
printf '%s  %s\n' "$CONTROL_MANIFEST_SHA256" "$OVERLAY_DIR/CONTROL.sha256" | \
  /usr/bin/sha256sum -c - >/dev/null || die 'v13 CONTROL.sha256 checksum mismatch'
(cd "$CACHE_ROOT" && /usr/bin/sha256sum -c "$OVERLAY_REL/CONTROL.sha256" >/dev/null) || \
  die 'v13 control-overlay content checksum mismatch'
printf '%s  %s\n' "$RUNNER_SHA256" "$OVERLAY_DIR/run-agent-omnibus.sh" | \
  /usr/bin/sha256sum -c - >/dev/null || die 'v13 runner checksum mismatch'
printf '%s  %s\n' "$PLATFORM_PATCH_SHA256" "$OVERLAY_DIR/agent-build-kylin-platform.patch" | \
  /usr/bin/sha256sum -c - >/dev/null || die 'v13 platform patch checksum mismatch'
printf '%s  %s\n' "$CONTROL_INFO_SHA256" "$OVERLAY_DIR/CONTROL-INFO" | \
  /usr/bin/sha256sum -c - >/dev/null || die 'v13 CONTROL-INFO checksum mismatch'
verify_patchelf_tool_authority
...
if ! /usr/bin/cmp -s -- "$OMNIBUS_SUCCESS" <(
  ...
  die 'Omnibus success handoff does not match the exact v7/v13 controls'
fi
