...
readonly EXPECTED_INTEGRATION_CORE_SHA=d725d9847379ff919b60a2f35e1f41af001e6054
...
readonly EXPECTED_CONTROL_OVERLAY_REL=control-overlays/62ad29793b02139448b76bc85fc406491a08bf58-7a4247599b029f1aca10d2cb63491d535fbd502f-aarch64-kylin10-v7-omnibus-kylin-platform-v13
readonly EXPECTED_CONTROL_OVERLAY_RUNNER_SHA256=c995773922ed242471e42e1e6e35460b48a7498bc531b7e028107d7b1321086d
readonly EXPECTED_PLATFORM_PATCH_SHA256=b4a5516b11029d2e225a02664b10677bb43a8dd8abd1afad587ee56ec93bccbe
readonly EXPECTED_CONTROL_INFO_SHA256=a06c295420edd7232438df2700c1a890c9b0bdd37269fd4cfd38fb4e2fb4e592
readonly EXPECTED_CONTROL_MANIFEST_SHA256=5491492ab454603d92a6f4de31fd1c13f47e34362eedcdf2f47e3b58cbc5a4d0
...
verify_omnibus_success_marker() {
  ...
  if ! cmp -s -- "$marker" <(
    ...
    die "omnibus.success does not match the exact Kylin v13 control handoff"
  fi
}
...
verify_omnibus_control_overlay() {
  ...
  [[ -d $overlay_dir && ! -L $overlay_dir ]] ||
    die "missing real Omnibus v13 control-overlay directory"
  [[ $(readlink -e -- "$overlay_dir") == "$overlay_dir" ]] ||
    die "Omnibus v13 control overlay resolves through an unexpected path"
  [[ $(stat -c '%u:%g:%a' -- "$overlay_dir") == 0:0:555 ]] ||
    die "Omnibus v13 control-overlay directory must be root:root mode 0555"
  ...
  [[ $actual_inventory == "$expected_inventory" ]] ||
    die "Omnibus v13 control-overlay inventory differs from the exact four-file set"
  ...
  for overlay_file in CONTROL-INFO CONTROL.sha256 agent-build-kylin-platform.patch; do
    [[ -f $overlay_dir/$overlay_file && ! -L $overlay_dir/$overlay_file ]] ||
      die "Omnibus v13 control file is not a real regular file: $overlay_file"
    [[ $(stat -c '%u:%g:%a' -- "$overlay_dir/$overlay_file") == 0:0:444 ]] ||
      die "Omnibus v13 control file must be root:root mode 0444: $overlay_file"
  done
  [[ -f $overlay_dir/run-agent-omnibus.sh && ! -L $overlay_dir/run-agent-omnibus.sh ]] ||
    die "Omnibus v13 runner is not a real regular file"
  [[ $(stat -c '%u:%g:%a' -- "$overlay_dir/run-agent-omnibus.sh") == 0:0:555 ]] ||
    die "Omnibus v13 runner must be root:root mode 0555"
  ...
  if ! cmp -s -- "$overlay_dir/CONTROL.sha256" <(
    ...
    die "Omnibus v13 CONTROL.sha256 does not match the pinned four-entry manifest"
  fi
  printf '%s  %s\n' "$EXPECTED_CONTROL_MANIFEST_SHA256" \
    "$overlay_dir/CONTROL.sha256" | sha256sum -c - >/dev/null ||
    die "Omnibus v13 CONTROL.sha256 digest differs from the pinned value"
  (
    cd "$cache_root"
    sha256sum -c "$EXPECTED_CONTROL_OVERLAY_REL/CONTROL.sha256"
  ) >/dev/null || die "Omnibus v13 control-overlay checksum verification failed"
...
preflight_bulk_workspace() {
  ...
  local capacity available_mib required_mib

  [[ $(stat -c '%d' -- "$scratch") == "$(stat -c '%d' -- "$destination")" ]] ||
    die "bulk scratch and OUTPUT_DIR must share a filesystem for atomic publication: $scratch vs $destination"
  ...
  log "bulk workspace preflight passed: available=${available_mib}MiB required=${required_mib}MiB runtime_nodes=$inode_count"
}
...
if [[ -e $archive_path || -L $archive_path ]]; then
  [[ -f $archive_path && ! -L $archive_path ]] || die "archive destination is not a regular file"
  [[ $(readlink -e -- "$archive_path") == "$archive_path" ]] ||
    die "existing archive destination is not canonical: $archive_path"
  cmp -s -- "$archive_pass1" "$archive_path" ||
    die "refusing to overwrite a different existing archive: $archive_path"
  [[ $(stat -c '%u:%g:%a' -- "$archive_path") == 0:0:644 ]] ||
    die "matching existing archive must be root:root mode 0644: $archive_path"
  remove_private_work_file 'redundant verified archive pass' "$archive_pass1" "$work_dir"
else
  chown root:root "$archive_pass1"
  chmod 0644 "$archive_pass1"
  sync -f -- "$archive_pass1"
  if ! ln -T -- "$archive_pass1" "$archive_path"; then
    die "archive destination appeared during atomic no-clobber publication: $archive_path"
  fi
  [[ -f $archive_path && ! -L $archive_path ]] ||
    die "atomic archive publication did not create a regular file: $archive_path"
  [[ $(readlink -e -- "$archive_path") == "$archive_path" ]] ||
    die "published archive destination is not canonical: $archive_path"
  [[ $(stat -c '%d:%i' -- "$archive_pass1") == "$(stat -c '%d:%i' -- "$archive_path")" ]] ||
    die "published archive is not the verified staged inode: $archive_path"
  [[ $(stat -c '%u:%g:%a' -- "$archive_path") == 0:0:644 ]] ||
    die "published archive must be root:root mode 0644: $archive_path"
  sync -f -- "$archive_path"
  remove_private_work_file 'published archive staging link' "$archive_pass1" "$work_dir"
  sync -f -- "$output_dir"
fi

sidecar_tmp="$work_dir/$archive_name.sha256"
printf '%s  %s\n' "$archive_sha256" "$archive_name" >"$sidecar_tmp"
chown root:root "$sidecar_tmp"
chmod 0644 "$sidecar_tmp"
if [[ -e $archive_path.sha256 || -L $archive_path.sha256 ]]; then
  [[ -f $archive_path.sha256 && ! -L $archive_path.sha256 ]] ||
    die "archive checksum destination is not a regular file: $archive_path.sha256"
fi
mv -f -T -- "$sidecar_tmp" "$archive_path.sha256"
[[ -f $archive_path.sha256 && ! -L $archive_path.sha256 ]] ||
  die "atomic checksum publication did not create a regular file: $archive_path.sha256"
[[ $(readlink -e -- "$archive_path.sha256") == "$archive_path.sha256" ]] ||
  die "published archive checksum destination is not canonical: $archive_path.sha256"
[[ $(stat -c '%u:%g:%a' -- "$archive_path.sha256") == 0:0:644 ]] ||
  die "published archive checksum must be root:root mode 0644: $archive_path.sha256"
