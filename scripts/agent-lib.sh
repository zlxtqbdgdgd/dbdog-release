...
agent_harvest_engine_passwords() { # <引擎 conf>；输出 port<TAB>password 对（每实例一行）
  # 同引擎多实例的监控密码可以各不相同（5432/5433 实测就是两套），升级路径
...
# dbdog 使用自己的私有摄入端；不要向任何公网 instrumentation intake
...
apm_config:
  ...
  # profiling 出口必须显式写。漏了不会静默外发（默认站点是 fail-closed 的
  # dbdog.invalid），而是变成 intake.profile.dbdog.invalid 的 DNS 失败 →
  # trace-agent 转发回 502 → ddprof 产出的 profile 全部丢弃。
  # 2026-08-11 实测：CH profiles 表长期 0 行，根因就是这里漏了一行。
